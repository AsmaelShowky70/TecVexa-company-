  ...
  return (
    <section id="services" className="relative py-20 lg:py-28 bg-slate-100/60 dark:bg-slate-950/60 border-t border-slate-200 dark:border-slate-800/80 transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14">
          ...
          <div className="mt-8 inline-flex p-1.5 rounded-2xl bg-slate-200 dark:bg-slate-900 border border-slate-300 dark:border-slate-800 shadow-md max-w-full overflow-x-auto">
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-xs sm:text-sm font-extrabold whitespace-nowrap transition-all ${
            ...
            {websiteTiers.map((pkg) => (
              <div
                ...
                className={`relative rounded-3xl p-6 sm:p-8 flex flex-col justify-between transition-all duration-300 ${
                  pkg.is_popular
                    ? 'bg-white dark:bg-gradient-to-b dark:from-slate-900 dark:to-slate-950 border-2 border-cyan-500 shadow-xl shadow-cyan-500/15 scale-105 z-20'
                    : 'glass-card border border-slate-200 dark:border-slate-800 hover:border-cyan-500/50'
                }`}
              >
                {/* Popular Badge */}
                {pkg.is_popular && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-gradient-to-r from-cyan-400 to-emerald-400 text-slate-950 text-xs font-black tracking-wider uppercase shadow-lg shadow-cyan-400/30">
                ...
                <div className="mt-8 pt-4">
                  <button
                    ...
                    className={`w-full py-3.5 px-4 rounded-xl text-sm font-extrabold transition-all flex items-center justify-center gap-2 shadow-lg ${
                      pkg.is_popular
                        ? 'bg-gradient-to-r from-cyan-400 to-emerald-400 text-slate-950 hover:from-cyan-300 hover:to-emerald-300 shadow-cyan-500/25 hover:scale-105'
        ...
        {activeTab === 'bundle' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {bundleOffers.map((pkg) => (
              <div
                key={pkg.id}
                className="relative rounded-3xl p-6 sm:p-8 glass-card border border-emerald-500/40 shadow-2xl shadow-emerald-500/10 flex flex-col justify-between group hover:border-emerald-400 transition-all"
              >
                <div>
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-xs font-bold px-3 py-1 rounded-lg bg-emerald-500/15 text-emerald-300 border border-emerald-500/30">
                      {lang === 'ar' ? pkg.badge_ar : pkg.badge_en}
                    </span>
                  ...
                  {/* Key Highlights */}
                  <div className="mt-5 space-y-2 p-3 rounded-2xl bg-slate-950/70 border border-slate-800 text-xs">
                    <div className="flex items-center gap-2 text-slate-300">
                      <Smartphone className="w-4 h-4 text-emerald-400 shrink-0" />
                      <span className="font-semibold text-white">{isRtl ? "نظام الموبايل:" : "Mobile System:"}</span>
                      <span>{isRtl ? "تطبيق أندرويد متكامل" : "Full Android Mobile App"}</span>
                    </div>
                    <div className="flex items-center gap-2 text-slate-300">
                      <Database className="w-4 h-4 text-cyan-400 shrink-0" />
                      <span className="font-semibold text-white">{isRtl ? "قاعدة البيانات:" : "Database:"}</span>
                      <span>{lang === 'ar' ? pkg.database_included_ar : pkg.database_included_en}</span>
                    </div>
                  </div>
                ...
                <div className="mt-8 pt-4">
                  <button
                    onClick={() => handlePackageWhatsApp(pkg)}
                    className="w-full py-3.5 px-4 rounded-xl text-sm font-extrabold bg-gradient-to-r from-emerald-400 to-teal-400 text-slate-950 hover:from-emerald-300 hover:to-teal-300 transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 hover:scale-105"
                  >
                    <MessageCircle className="w-4 h-4" />
                    <span>{t.pricing.choosePlan}</span>
                    <ArrowRight className={`w-3.5 h-3.5 ${isRtl ? 'rotate-180' : ''}`} />
        ...
        {activeTab === 'bespoke' && (
          <div className="max-w-4xl mx-auto">
            {bespokeSystems.map((pkg) => (
              <div
                key={pkg.id}
                className="rounded-3xl p-8 sm:p-10 glass-card border border-purple-500/40 shadow-2xl shadow-purple-500/10 group"
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <span className="text-xs font-bold px-3 py-1.5 rounded-lg bg-purple-500/20 text-purple-300 border border-purple-500/40">
                    {lang === 'ar' ? pkg.badge_ar : pkg.badge_en}
                  </span>
                  <span className="text-xs text-slate-400 font-mono">
                    {pkg.billing_period}
                ...
                <div className="mt-6 pt-5 border-t border-slate-800 flex items-baseline gap-3">
                ...
                <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {(lang === 'ar' ? pkg.features_ar : pkg.features_en).map((feat, idx) => (
                    <div key={idx} className="flex items-start gap-2.5 text-sm text-slate-300 p-2.5 rounded-xl bg-slate-950/60 border border-slate-800">
                ...
                <div className="mt-8 pt-4">
                  <button
                    onClick={() => handlePackageWhatsApp(pkg)}
                    className="w-full py-4 px-6 rounded-xl text-base font-extrabold bg-gradient-to-r from-purple-500 via-indigo-500 to-cyan-500 text-white hover:opacity-95 transition-all flex items-center justify-center gap-2 shadow-xl shadow-purple-500/25 hover:scale-105"
                  >
        ...
        <div className="mt-16 rounded-3xl p-6 sm:p-10 glass-card border border-slate-200 dark:border-slate-700/80 max-w-4xl mx-auto shadow-xl relative overflow-hidden">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-xl bg-cyan-500/10 dark:bg-cyan-500/20 border border-cyan-500/30 flex items-center justify-center text-cyan-600 dark:text-cyan-400">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mb-8">
            ...
            <label className={`p-4 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${
              calcWeb ? 'bg-cyan-500/10 dark:bg-cyan-500/15 border-cyan-500/50 text-slate-900 dark:text-white font-bold' : 'bg-slate-50 dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-400'
            }`}>
              <div className="flex items-center gap-3">
                ...
                />
                <span className="text-sm font-semibold">{t.pricing.optWeb}</span>
              </div>
              <span className="text-xs font-mono text-cyan-700 dark:text-cyan-300 font-bold">+3,500 ج.م</span>
            </label>

            <label className={`p-4 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${
              calcAndroid ? 'bg-emerald-500/10 dark:bg-emerald-500/15 border-emerald-500/50 text-slate-900 dark:text-white font-bold' : 'bg-slate-50 dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-400'
            }`}>
              <div className="flex items-center gap-3">
                <input 
                  type="checkbox" 
                ...
                />
                <span className="text-sm font-semibold">{t.pricing.optAndroid}</span>
              </div>
              <span className="text-xs font-mono text-emerald-700 dark:text-emerald-300 font-bold">+9,000 ج.م</span>
            </label>

            <label className={`p-4 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${
              calcPaidDomain ? 'bg-blue-500/10 dark:bg-blue-500/15 border-blue-500/50 text-slate-900 dark:text-white font-bold' : 'bg-slate-50 dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-400'
            }`}>
              <div className="flex items-center gap-3">
                <input 
                  type="checkbox" 
                ...
                />
                <span className="text-sm font-semibold">{t.pricing.optDomain}</span>
              </div>
              <span className="text-xs font-mono text-blue-700 dark:text-blue-300 font-bold">+1,200 ج.م</span>
            </label>

            <label className={`p-4 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${
              calcPaidHosting ? 'bg-indigo-500/10 dark:bg-indigo-500/15 border-indigo-500/50 text-slate-900 dark:text-white font-bold' : 'bg-slate-50 dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-400'
            }`}>
              <div className="flex items-center gap-3">
                <input 
                  type="checkbox" 
                ...
                />
                <span className="text-sm font-semibold">{t.pricing.optPaidHost}</span>
              </div>
              <span className="text-xs font-mono text-indigo-700 dark:text-indigo-300 font-bold">+2,200 ج.م</span>
            </label>

            <label className={`p-4 rounded-xl border flex items-center justify-between cursor-pointer transition-all sm:col-span-2 ${
              calcPaidDb ? 'bg-teal-500/10 dark:bg-teal-500/15 border-teal-500/50 text-slate-900 dark:text-white font-bold' : 'bg-slate-50 dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-400'
            }`}>
              <div className="flex items-center gap-3">
                <input 
                  type="checkbox" 
                ...
                />
                <span className="text-sm font-semibold">{t.pricing.optDb}</span>
              </div>
              <span className="text-xs font-mono text-teal-700 dark:text-teal-300 font-bold">+1,900 ج.م</span>
            </label>

          </div>

          {/* Calculator Bottom Summary */}
          <div className="p-5 rounded-2xl bg-white dark:bg-slate-950/90 border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
            <div>
            ...
            <button
              ...
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl font-extrabold text-slate-950 bg-gradient-to-r from-cyan-400 to-emerald-400 hover:from-cyan-300 hover:to-emerald-300 shadow-lg shadow-cyan-500/25 transition-all hover:scale-105"
            >
